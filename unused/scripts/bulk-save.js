...
function stopSaving() {
  ...
  $("#list-container").off("click");
  $("#main-info").text(
    "Done - Click on a link to view on the Wayback Machine.",
  );
}
...
function doBulkSaveAll(e) {
  ...
  if ($("#chk-screenshot").prop("checked") === true) {
    saveOptions["capture_screenshot"] = 1;
  }
  const savedWithin = $("#saved-within-opt").val() || "99999d";
  if ($("#saved-within-chk").prop("checked") === true) {
    saveOptions["if_not_archived_within"] = savedWithin;
  }
...
function updateLimitCount() {
  ...
  if (bulkSaveLimit === 0) {
    ...
    // show count limits
    $("#limit-count-info").text(
      `${totalUrlCount} of ${bulkSaveLimit} max limit`,
    );
...
function processStatus(msg, url, wbUrl) {
  ...
  if (
    curl &&
    bulkSaveMap.has(curl) &&
    bulkSaveMap.get(curl).status !== S_DONE
  ) {
    let obj = bulkSaveMap.get(curl);
    let $row = obj.row;
    if ($row) {
...
function checkIfFinished() {
  if (
    totalUrlCount > 0 &&
    saveSuccessCount + saveFailedCount >= totalUrlCount
  ) {
    stopSaving();
    if (saveFailedCount === 0) {
      $("#copy-unsaved-btn").attr("disabled", true);
    }
  }
}
...
function doCopyAll() {
  let text = Array.from(bulkSaveMap.values())
    .map((obj) => obj.url)
    .join("\n");
...
$(function () {
  if (chrome.bookmarks) {
    ...
    $("#main-info").text(
      "Enter a list of URLs below or import from your bookmarks.",
    );
  } else {
