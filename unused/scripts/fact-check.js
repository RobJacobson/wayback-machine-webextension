...
function runFactCheck() {
  ...
  if (isValidUrl(url) && isNotExcludedUrl(url)) {
    chrome.runtime.sendMessage(
      { message: "getFactCheckResults", url: url },
      (resp) => {
        ...
        if (resp && resp.json) {
          if (
            resp.json.results &&
            resp.json.results.meta &&
            resp.json.results.meta.indicators
          ) {
            // api returns results
            let item = resp.json.results.meta;
            for (let i = 0; i < item.indicators.length; i++) {
              let row = $('<div class="fact-checks-list">');
              let checkedBy = $('<div class="name">').text(
                item.indicators[i].name,
              );
              let relatedAnalysis = $('<a target="_blank">')
                .text(item.indicators[i].url)
                .attr("href", item.indicators[i].url);
              /*
            ...
            */
            ...
            $("#results-container").append(
              row.text("No fact checks found for the current URL."),
            );
          }
        } else {
          // api call failed
          let row = $('<div class="fact-checks-list title">');
          $("#results-container").append(
            row.text("Fact check server is not responding."),
          );
        }
      },
    );
