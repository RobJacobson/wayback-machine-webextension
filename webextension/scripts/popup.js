...
function showSettingsTabTip() {
  let tt = $("<div>").append(
    $("<p>")
      .text("There are more great features in Settings!")
      .attr({ class: "setting-tip" }),
  )[0].outerHTML;
  let tabItem = $("#settings-tab-btn");
  setTimeout(() => {
    tabItem
      .append(attachTooltip(tabItem, tt, "top"))
      .tooltip("show")
      .on("mouseenter", () => {
          ...
          (message) => {
            ...
            if (message) {
              if ("last_ts" in message && message.last_ts) {
                $("#last-saved-msg")
                  .text("Last Saved " + viewableTimestamp(message.last_ts))
                  .show();
...
function loginError() {
  ...
  $("#login-tab-btn")
    ...
    .off("click")
    .on("click", showLoginFromTab);

  // setup messages
  if (activeURL && !isNotExcludedUrl(activeURL)) {
    showUrlNotSupported(true);
  }
}
...
function openNewestPage() {
  openWaybackPage("https://web.archive.org/web/2/", activeURL);
}
function openOldestPage() {
  openWaybackPage("https://web.archive.org/web/0/", activeURL);
}
    ...
    if (id.includes("facebook-share-btn")) {
      openByWindowSetting(
        "https://www.facebook.com/sharer.php?u=" +
          fixedEncodeURIComponent(sharing_url),
      );
...
function searchTweet() {
  ...
  if (isValidUrl(clean_url)) {
    ...
    if (curl) {
      ...
      let open_url =
        "https://twitter.com/search?q=" + fixedEncodeURIComponent(query);
  ...
  search_box.addEventListener("keydown", (e) => {
    ...
    if (e.key === "ArrowUp") {
      if (index === list.firstChild && index && list.lastChild) {
        if (index.classList.contains("focused")) {
...
function showLoginFromMain(e) {
  $("#login-label").html(
    "This feature requires logging in to the Internet Archive",
  );
  $(".back-btn").off("click").on("click", goBackToMain);
  showLoginPage(e);
}
...
function setupViewArchived() {
  chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
    if (tabs && tabs[0]) {
      chrome.runtime.sendMessage(
        { message: "getToolbarState", atab: tabs[0] },
        (result) => {
          checkLastError();
          const state =
            result && "stateArray" in result
              ? new Set(result.stateArray)
              : new Set();
          if (
            state.has("V") &&
            "customData" in result &&
            "statusWaybackUrl" in result.customData &&
            "statusCode" in result.customData &&
            result.customData.statusCode >= 400 &&
            "statusUrl" in result.customData &&
            cropPrefix(result.customData.statusUrl) === cropPrefix(tabs[0].url)
          ) {
            ...
            const statusText =
              ...
              (ERROR_CODE_DIC[statusCode] || "Error");
            $("#last-saved-msg").hide();
      ...
      if (url.includes("www.amazon") && url.includes("/dp/")) {
        chrome.runtime.sendMessage(
          ...
          (result) => {
            checkLastError();
            let state =
              result && result.stateArray
                ? new Set(result.stateArray)
                : new Set();
            if (state.has("R")) {
              $("#readbook-container").show();
              chrome.storage.local.get(
                ["tab_url", "detail_url", "view_setting"],
                (settings) => {
                  if (!settings) {
                    return;
                  }
                  ...
                  if (stored_url === url) {
                    ...
                    fetch(hostURL + "services/context/amazonbooks?url=" + url, {
                      ...
                      .then((response) => {
                        if (
                          response["metadata"] &&
                          response["metadata"]["identifier-access"]
                        ) {
                          const new_details_url =
                            response["metadata"]["identifier-access"];
                          $("#readbook-btn").click(() => {
              ...
              $("#tvnews-btn").click(() => {
                chrome.storage.local.get(["view_setting"], function (settings) {
                  if (settings && settings.view_setting) {
                    const URL =
                      chrome.runtime.getURL("tvnews.html") + "?url=" + url;
        ...
        chrome.runtime.sendMessage(
          ...
          (result) => {
            ...
            if (state.has("R")) {
              ...
              $("#wikibooks-btn").click(() => {
                const URL =
                  chrome.runtime.getURL("cited-books.html") +
                  "?url=" +
              ...
              $("#wikipapers-btn").click(() => {
                const URL =
                  chrome.runtime.getURL("cited-papers.html") +
                  "?url=" +
                  fixedEncodeURIComponent(url);
                openByWindowSetting(URL);
              });
          ...
          chrome.runtime.sendMessage(
            ...
            (result) => {
              ...
              if (
                state.has("F") &&
                "customData" in result &&
                "contextUrl" in result.customData
              ) {
  ...
  if (url && isValidUrl(url)) {
    if (id.includes("alexa-btn")) {
      ...
      const annotationsUrl =
        chrome.runtime.getURL("annotations.html") + "?url=" + url;
...
function showWaybackCount(url) {
  ...
  chrome.runtime.sendMessage(
    { message: "getCachedWaybackCount", url: url },
    (result) => {
      checkLastError();
  ...
  chrome.runtime.onMessage.addListener((message) => {
    if (activeURL === message.url) {
      if (message.message === "save_success") {
        ...
        $("#last-saved-msg")
          .text("Last Saved " + viewableTimestamp(message.timestamp))
          .show();
        $("#spn-btn").removeClass("flip-inside");
        setupWaybackCount();
        enableAfterSaving();
