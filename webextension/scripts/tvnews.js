...
function constructArticles(clip) {
  let topElements = $("<div>")
    .addClass("top_elements")
    .append(
      $("<p>")
        .text(clip.show)
        .prepend($("<strong>").text(clip.station + ": ")),
    );
  let bottomElements = $("<div>")
    .addClass("bottom_elements")
    .append(
      $("<a>")
        .attr({ href: clip.preview_url })
        .append(
  ...
  return $("<div>").append(topElements, bottomElements);
}
...
function getArticles(url) {
  getDetails(url)
    ...
    .catch((err) => {
      ...
      $("#RecommendationTray")
        .css({ "grid-template-columns": "none" })
        .append($("<p>").text(err).css({ margin: "auto" }));
    });
}
