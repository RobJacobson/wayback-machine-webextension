...
function populateBooks(url) {
  ...
  getWikipediaBooks(url)
    .then((data) => {
      $(".loader").hide();
      if (
        data &&
        data.message !== "No ISBNs found in page" &&
        data.status !== "error"
      ) {
    ...
    .catch((error) => {
      ...
      $("#resultsTray")
        .css("grid-template-columns", "none")
        .append($("<div>").html(error));
    });
}

function addBook(metadata) {
  let text_elements = $("<div>")
    .attr({ class: "text-elements" })
    .append($("<h3>").text(metadata.title), $("<p>").text(metadata.author));
  let details = $("<div>")
    .attr({ class: "bottom-details" })
    .append(
      metadata.image
        ? $("<img>").attr({ class: "cover-img", src: metadata.image })
        : $("<p>").attr({ class: "cover-img" }).text("No cover available"),
      $("<button>")
        .attr({ class: metadata.button_class })
        .text(metadata.button_text)
        .click(() => {
