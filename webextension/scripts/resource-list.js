...
$(function () {
  ...
  if (document.location.hash === "#not_refreshed") {
    ...
    showError(
      "Looks like you have reloaded the page. Please close and try again!",
    );
  }
});
  ...
  chrome.runtime.onMessage.addListener((message) => {
    if (message.message === "resource_list_show" && message.url === url_name) {
      ...
      if (new_resource_length > old_resource_length) {
        for (let item of Array.from([...resource_list_data]).slice(
          old_resource_length,
          new_resource_length,
        )) {
          $("#resource-list-container").append($("<p>").append(item));
        }
      ...
      if (status === "success") {
        ...
        let snapshot_url =
          ...
          vdata.original_url;
        $("#snapshot-url")
          .text("Click to view snapshot")
          .attr("href", snapshot_url)
          .show();
      ...
      message.message === "resource_list_show_error" &&
