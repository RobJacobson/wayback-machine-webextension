...
function doLogin(e) {
  ...
  chrome.cookies.set(
    { url: "https://archive.org", name: "test-cookie", value: "1" },
    (cookie) => {
      if (!cookie) {
        // failed to set cookie
        console.log("Cookie access is blocked. Please check cookie settings.");
        console.log(
          "This could happen if 'privacy.firstparty.isolate' is set to true.",
        );
      ...
      if (res.success === false) {
        ...
        checkAuthentication((result) => {
          ...
          if (result && result.auth_check) {
            ...
            console.log(
              "Cookie access is blocked. Please check cookie settings.",
            );
            $("#login-message").show().text("Please Enable Cookies");
            $("#login-btn").val("Login");
          }
