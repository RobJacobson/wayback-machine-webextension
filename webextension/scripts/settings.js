...
function restoreSettings(items) {
  ...
  $(`input[name=view-setting-input][value=${items.view_setting}]`).prop(
    "checked",
    true,
  );
  // update UI
...
function setupPanelSwitch() {
  if (!$("#panel2-btn").hasClass("selected")) {
    $("#panel2-btn").addClass("selected");
  }
  $("#first-panel").hide();
  $("#panel1-btn").click(() => {
    $("#second-panel").hide();
    $("#first-panel").show();
    if (!$("#panel1-btn").hasClass("selected")) {
      $("#panel1-btn").addClass("selected");
    }
    if ($("#panel2-btn").hasClass("selected")) {
      $("#panel2-btn").removeClass("selected");
    }
  });
  $("#panel2-btn").click(() => {
    $("#first-panel").hide();
    ...
    if ($("#panel1-btn").hasClass("selected")) {
      $("#panel1-btn").removeClass("selected");
    }
  });
}
...
function switchTabWindow() {
  let idx = 0,
    inputs = $("input[name=view-setting-input]");
...
function setupHelpDocs() {
  let docs = {
    ...
    "private-mode-setting":
      "Don't initiate automatic communications between this extension and the Internet Archive.",
    "not-found-setting":
      "If server returns a 4xx or 5xx then check the Wayback Machine for archives.",
    "wm-count-setting":
      "Show number of times the current URL has been archived in the Wayback Machine.",
    "fact-check-setting":
      "Check for contextual information from fact checking organizations and origin websites.",
    "wiki-setting":
      "Check for books and academic papers from the Internet Archive referenced in Wikipedia articles. ",
    "amazon-setting":
      "Check if books from Amazon are available from the Internet Archive.",
    "tvnews-setting":
      "Auto check for related TV News Clips while visiting selected news websites.",
    // general tab
    "auto-archive-setting":
      "Archive URLs that have not previously been archived to the Wayback Machine.",
    "auto-bookmark-setting":
      "Archive when bookmarking a website, except Excluded URLs.",
    "email-outlinks-setting":
      "Send an email of results when Outlinks option is selected.",
    "my-archive-setting":
      "Adds URL to My Web Archive when Save Page Now is selected.",
    "notify-setting": "Turn off all notifications.",
    "resource-list-setting":
      "Display embedded URLs archived with Save Page Now.",
  };
  let labels = $("label");
  for (let i = 0; i < labels.length; i++) {
    ...
    if (docs[docFor]) {
      let tt = $("<div>").append(
        $("<p>").text(docs[docFor]).attr({ class: "setting-tip" }),
      )[0].outerHTML;
