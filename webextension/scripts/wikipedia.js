...
(function (window) {
  ...
  function getBookAnchorElements() {
    ...
    for (const node of nodes) {
      if (isISBN(extractISBN(node.href))) {
        books.push(node);
      }
    }
    return books;
      ...
      .then((data) => {
        if (!data || (data && data.status && data.status === "error")) {
          ...
          chrome.runtime.sendMessage(
            { message: "addToolbarStates", states: ["R", "books"], url: url },
            (result) => {
  ...
  function getMetadata(book) {
    ...
    if (book && book.metadata) {
      return {
        title:
          book.metadata.title.length > MAX_TITLE_LEN
            ? book.metadata.title.slice(0, MAX_TITLE_LEN) + "..."
            : book.metadata.title,
  ...
  function addDonateIcon(isbn) {
    return attachTooltip(createDonateAnchor(isbn), createDonateToolTip(isbn));
  }
  ...
  function createReadToolTip(id, metadata) {
    return (
      `<a href="https://www.archive.org/details/${id}" target="_blank">
      <div class="wm1996-tooltip-header">
      ...
      <div class="wm1996-tooltip-details">` +
      (metadata.image
        ? `<div class="wm1996-book-outer"><img class="wm1996-book-img" src="${metadata.image}" alt="Read Book"></div>`
        : "") +
  ...
  function getIdentifier(book) {
    ...
    if (book && book.metadata) {
      return book.metadata.identifier;
    }
    return null;
  }
  ...
  function attachTooltip(anchorHtml, tooltipHtml) {
    ...
    span.addEventListener("mouseover", (e) => {
      ...
      if (x + halfw > window.innerWidth) {
        x = window.innerWidth - halfw;
      }
      div.style.left = `${x}px`;
      div.style.top = `${y}px`;
      // remove popup
